...
class VideoCarousel extends Component {
  ...
  shiftCarousel(direction) {
    ...
    posterSelected =
      posterSelected === -1 ? posters.length - 1 : posterSelected;
  ...
  shouldComponentUpdate() {
    ...
    if (this.props.home.videos.totalCount > 0) {
      ...
      const rightTailValueExceedsPostersLength =
        this.state.carousel.right >= this.props.home.videos.totalCount;
      const leftTailValueExceedsPostersLength =
        this.state.carousel.left >= this.props.home.videos.totalCount;
      switch (true) {
        case rightTailValueExceedsPostersLength &&
          leftTailValueExceedsPostersLength:
          this.setState({
            carousel: {
  ...
  render() {
    ...
    const renderPosters = (posters, left, right) => {
      const generateCarouselIndices = (length, lowerBound, upperBound) => {
        ...
        // console.log(length);
        let orderedIndex = lowerBound,
          carouselIndex = 0;
        const tailsOfOrderedIndicesLoaded = lowerBound > upperBound;
        const rangeInsideOrderedIndices =
          upperBound > lowerBound && upperBound < length;
        let carouselIndices = [],
          tailLength;
        switch (true) {
      ...
      const posterIndices = generateCarouselIndices(
        posters.length,
        left,
        right
      );
    ...
    const initialisePosters = (posters, left, right) => {
      ...
      if (waitingForPostersToLoad) {
        ...
        return (
          <ul className="posters">{renderPosters(posters, left, right)}</ul>
        );
      }
    };
    ...
    return (
      <div
        ...
        onWheel={e => this.wheelThroughCarousel(e)}
        onKeyDown={e => this.keyThroughCarousel(e)}
      >
        {initialisePosters(
          listOfPosters,
          this.state.carousel.left,
          this.state.carousel.right
        )}
